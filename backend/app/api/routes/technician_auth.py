...
def _new_token(technician_id: str, username: str) -> str:
    payload = f"{technician_id}:{username}:{int(time.time()) + 12 * 60 * 60}"
    return f"{payload}:{_signature(payload)}"


def _valid_token(token: str) -> dict | None:
    try:
        technician_id, username, expires, signature = token.rsplit(":", 3)
        payload = f"{technician_id}:{username}:{expires}"
        if int(expires) < int(time.time()) or not hmac.compare_digest(signature, _signature(payload)):
            return None
        return technician_store.get_active(technician_id, username)
...
async def technician_login(request: LoginRequest) -> dict:
    technician = technician_store.authenticate(request.username, request.password)
    if technician is None:
        raise HTTPException(401, "invalid_technician_credentials")
    return {
        "access_token": _new_token(technician["id"], technician["username"]),
        "token_type": "bearer",
        "expires_in": 12 * 60 * 60,
